...
export function StaffIdMissingNotice() {
  return (
    <div className="mx-auto max-w-lg rounded-xl border border-amber-200 bg-amber-50 px-5 py-4 text-left text-sm leading-relaxed text-amber-950">
      ...
      <p className="mt-2">
        ログインセッションに職員ID（スタッフID）が含まれていません。在籍マスタで氏名・スタッフID
        と、アカウント作成時や登録で使った名前が一致しているか確認してください。氏名には「齊／斉／斎」など表記ゆれがあるため、ワークスペースメール同一の場合は当月シートに「メール」列へ登録してください。
      </p>
      <p className="mt-3">
        <Link href="/" className="font-medium text-orange-800 underline underline-offset-2">
          サイトトップへ戻る
        </Link>
      </p>
    </div>
